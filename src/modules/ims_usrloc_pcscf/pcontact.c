...
{
	LM_DBG("linking IMPU <%.*s> to contact <%.*s>\n", _p->public_identity.len,
			_p->public_identity.s, _c->aor.len, _c->aor.s);
		...
		struct pcontact **_c)
{
	...
	*_c = (pcontact_t *)shm_malloc(sizeof(pcontact_t) + _contact->len
								   + _ci->received_host.len
								   + _ci->via_host.len);
	if(*_c == 0) {
	...
	LM_DBG("New contact [<%.*s>] with %d associated IMPUs in state: [%s]\n",
			_contact->len, _contact->s, _ci->num_public_ids,
...
static inline void nodb_timer(pcontact_t *_c)
{
	LM_DBG("Running nodb timer on <%.*s>, "
		   ...
		   "Slot: %u\n",
			...
			(int)_c->expires, (int)(_c->expires - time(NULL)),
			_c->received_host.len, _c->received_host.s, _c->received_port,
			_c->path.len, _c->path.s, _c->received_proto, _c->aorhash, _c->sl);

	get_act_time();


	if((_c->expires - act_time) + expires_grace
			<= 0) { //we've allowed some grace time TODO: add as parameter
		...
		if(db_mode == DB_ONLY) {
			if(db_load_pcontact(
					   _c->slot->d, &_c->aor, 0 /*insert_cache*/, &_c, NULL)) {
				if((_c->reg_state == PCONTACT_REG_PENDING_AAR)
						|| (_c->reg_state == PCONTACT_REG_PENDING)) {
