...
static int mod_init(void)
{
	...
	init_dlg_handlers(rr_param, dlg_flag, timeout_spec.s ? &timeout_avp : 0,
			default_timeout, seq_match_mode);
...
static int w_set_dlg_profile(struct sip_msg *msg, char *profile, char *value)
{
	...
	if(((struct dlg_profile_table *)profile)->has_value) {
		if(pve == NULL || pv_printf_s(msg, pve, &val_s) != 0 || val_s.len == 0
				|| val_s.s == NULL) {
			LM_WARN("cannot get string for value\n");
			return -1;
		}
		if(set_dlg_profile(msg, &val_s, (struct dlg_profile_table *)profile)
				< 0) {
			LM_ERR("failed to set profile");
			return -1;
		}
	} else {
		if(set_dlg_profile(msg, NULL, (struct dlg_profile_table *)profile)
				< 0) {
			LM_ERR("failed to set profile");
			return -1;
		}
...
static int w_unset_dlg_profile(struct sip_msg *msg, char *profile, char *value)
{
	...
	if(((struct dlg_profile_table *)profile)->has_value) {
		...
		if(unset_dlg_profile(msg, &val_s, (struct dlg_profile_table *)profile)
				< 0) {
			LM_ERR("failed to unset profile");
			return -1;
		}
	} else {
		if(unset_dlg_profile(msg, NULL, (struct dlg_profile_table *)profile)
				< 0) {
			LM_ERR("failed to unset profile");
			return -1;
		}
...
static int w_is_in_profile(struct sip_msg *msg, char *profile, char *value)
{
	...
	if(pve != NULL && ((struct dlg_profile_table *)profile)->has_value) {
		...
		return is_dlg_in_profile(
				msg, (struct dlg_profile_table *)profile, &val_s);
	} else {
		return is_dlg_in_profile(
				msg, (struct dlg_profile_table *)profile, NULL);
	}
}
...
static inline void internal_rpc_print_dlg(
		rpc_t *rpc, void *c, struct dlg_cell *dlg, void *dh)
{
	void *dlg_outs_h;
	void *root;
	void *roota;
	...
	while(dlg_out) {
		...
		rpc->struct_add(dlg_outs_h, "dd", "Entry", dlg_out->h_entry, "Id",
				dlg_out->h_id);
...
static void internal_rpc_print_dlgs(rpc_t *rpc, void *c)
{
	...
	if(rpc->struct_add(ah, "d[", "Size", (int)d_table->size, "Dialogs", &dh)
			< 0) {
...
static void rpc_end_dlg_entry_id(rpc_t *rpc, void *c)
{
	...
	LM_DBG("Setting reason to [%.*s] and extra headers to [%.*s]\n", reason.len,
			reason.s, rpc_extra_hdrs.len, rpc_extra_hdrs.s);
	dlg_terminate(dlg, NULL, &reason /*reason*/, 2, &rpc_extra_hdrs);
}

static void rpc_end_all_active_dlg(rpc_t *rpc, void *c)
{
	struct dlg_cell *dlg = NULL, *dlg_to_term = NULL;
	dlg_entry_t entry;

	unsigned int i_count = 0, h_index = 0;
	...
	for(h_index = 0; h_index < d_table->size; h_index++) {
		...
		for(; dlg_to_term;) {
			...
			if(dlg_to_term->state == DLG_STATE_CONFIRMED) {
				...
				rpc_extra_hdrs = dfl_rpc_extra_hdrs;
				dlg_terminate(dlg_to_term, NULL, &reason /*reason*/, 2,
						&rpc_extra_hdrs);
				i_count++;
			}
			dlg_to_term = dlg;
