...
extern int
		store_data_on_dereg; /**< should we store SAR user data on de-registration  */
...
static inline int calc_contact_expires(
		contact_t *c, int expires_hdr, int sos_reg)
{
	int r = 0;
	if(c && c->expires)
 ...
 */
static inline ucontact_info_t *pack_ci(
		struct sip_msg *_m, contact_t *_c, unsigned int _e, unsigned int _f)
{
...
static inline int is_impu_registered(udomain_t *_d, str *public_identity)
{
	...
	if(res != 0) {
		...
		if((impu->linked_contacts.numcontacts <= 0)
				|| (impu->linked_contacts.head == 0)) {
				...
				if(h->type == HDR_CONTACT_T && h->parsed) {
					...
					for(chi = ((contact_body_t *)h->parsed)->contacts; chi;
						...
						LM_DBG("Need to update contact: <%.*s>: "
							   ...
							   "sos: [%d],"
							   "expires [%" TIME_T_FMT "]\n",
								chi->uri.len, chi->uri.s, qvalue, sos,
								TIME_T_CAST(expires - time(NULL)));
						...
						if(result != 0) { //get_contact returns with lock
							...
							if(ucontact->state != CONTACT_VALID) {
								LM_WARN("contact is not in state valid - this "
										"is a race between dereg and "
										"reg/re-reg");
								ucontact->state =
										CONTACT_VALID; //TODO this should prob move into the contact info structure - ie pass state into update
							}
							if(ul.update_ucontact(impu_rec, ucontact, ci)
									!= 0) {
...
int store_explicit_dereg_contact(struct sip_msg *msg,
		str **explicit_dereg_contact, int *num_explicit_dereg_contact)
{
	...
	LM_DBG("This is an explicit deregistration so we need to store the contact "
		   "URI to send out NOTIFY\n");
		...
		if(h->type == HDR_CONTACT_T && h->parsed) {
			for(chi = ((contact_body_t *)h->parsed)->contacts; chi;
				...
				LM_DBG("URI [%.*s] and current bytes needed [%d]\n",
						chi->uri.len, chi->uri.s,
						bytes_needed_explicit_dereg_contact);
			}
		}
	}
	...
	len_num_explicit_dereg_contact =
			(sizeof(str) * (*num_explicit_dereg_contact))
			+ bytes_needed_explicit_dereg_contact;
	...
	*explicit_dereg_contact = (str *)shm_malloc(len_num_explicit_dereg_contact);
	...
	for(h = msg->contact; h; h = h->next) {
		if(h->type == HDR_CONTACT_T && h->parsed) {
			for(chi = ((contact_body_t *)h->parsed)->contacts; chi;
					chi = chi->next) {
				LM_DBG("Adding [%.*s] to list of explicit contacts that have "
					   "been de-reged\n",
						chi->uri.len, chi->uri.s);
		...
		struct sip_msg *_m, char *str1, str *direction, char *route)
{
	...
	LM_DBG("Setting default AVP return code used for async callbacks to "
		   "default as ERROR \n");
	...
	if(scscf_assign_server(_m, public_identity, private_identity,
			   assignment_type, data_available, saved_t)
			!= 0) {
...
int save(struct sip_msg *msg, char *str1, char *route, int _cflags)
{
	...
	if(expires
			!= 0) { //if <0 then no expires was found in which case we treat as reg/re-reg with default expires.
		...
		if(store_data_on_dereg) {
			LM_DBG("preparing for SAR assignment for DE-REGISTRATION with "
				   "storage <%.*s>\n",
					public_identity.len, public_identity.s);
			sar_assignment_type =
		...
		if(!st) { //if it is a star then we delete all contacts and send a SAR
			...
			if(res <= 0) {
				...
				//send sar
				LM_DBG("no contacts left after explicit de-registration, doing "
					   "SAR\n");
			} else { //res=1
				//still contacts left so return success
				LM_DBG("contacts still available after deregister.... not "
					   "doing SAR\n");
	...
	if(scscf_assign_server(msg, public_identity, private_identity,
			   sar_assignment_type, data_available, saved_t)
			!= 0) {
		LM_ERR("ERR:I_MAR: Error sending SAR or SAR time-out\n");
		tmb.t_cancel_suspend(saved_t->tindex, saved_t->tlabel);
		free_saved_transaction_data(saved_t);
...
no_sar:
	...
	if((is_route_type(REQUEST_ROUTE))
			&& (reg_send_reply(msg, contact_header) < 0)) {
		free_contact_buf(contact_header);
		return CSCF_RETURN_ERROR;
	}
	free_contact_buf(contact_header);
