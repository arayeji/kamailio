		...
		str *_dom, str *_aor, str *_contact, ucontact_info_t *_ci)
{
	...
	while(param) {
		...
		curr->next = 0;
		if(param->body.len > 0 && param->body.s) {
			if(shm_str_dup(&curr->body, &param->body) < 0)
				goto error;
		} else {
			curr->body.s = 0;
			curr->body.len = 0;
		...
		if(param->name.len > 0 && param->name.s) {
			if(shm_str_dup(&curr->name, &param->name) < 0)
				goto error;
		} else {
			curr->name.s = 0;
			curr->name.len = 0;
	...
	if(_aor && _aor->len > 0) {
		if(shm_str_dup(&c->aor, _aor) < 0)
			goto error;
	}
	if(_dom && _dom->len > 0) {
		if(shm_str_dup(&c->domain, _dom) < 0)
			goto error;
	}
...
void print_ucontact(FILE *_f, ucontact_t *_c)
{
	...
	while(tmp) {
		fprintf(_f, "\t\t\tParam Name: '%.*s' Param Body '%.*s'\n",
				tmp->name.len, ZSW(tmp->name.s), tmp->body.len,
				ZSW(tmp->body.s));
		tmp = tmp->next;
	}
...
#define update_str(_old, _new)                         \
	...
	LM_DBG("Setting contact expires to %d which is in %d seconds time\n",
			(unsigned int)_ci->expires,
			(unsigned int)(_ci->expires - time(NULL)));
	_c->sock = _ci->sock;
...
int expire_scontact(struct impurecord *_r, ucontact_t *_c)
{
	...
	if(db_mode == WRITE_THROUGH
			&& (db_insert_ucontact(_r, _c)
					!= 0)) { /* this is an insert/update */
		...
		ucontact_t *_c, unsigned int h_entry, unsigned int h_id)
{
	...
	struct contact_dialog_data *dialog_data =
			(struct contact_dialog_data *)shm_malloc(
					sizeof(struct contact_dialog_data));

	LM_DBG("Adding dialog data to contact <%.*s> with h_entry <%d> and h_id "
		   "<%d>",
			_c->c.len, _c->c.s, h_entry, h_id);
...
{
	struct contact_dialog_data *dialog_data, *tmp_dialog_data;
	LM_DBG("Removing dialog data from contact <%.*s> with h_entry <%d> and "
		   "h_id <%d>",
			_c->c.len, _c->c.s, h_entry, h_id);
	...
	for(dialog_data = _c->first_dialog_data; dialog_data;) {
		tmp_dialog_data = dialog_data;
		dialog_data = dialog_data->next;
		if(tmp_dialog_data->h_entry == h_entry
				&& tmp_dialog_data->h_id == h_id) {
