...
#define IMS_USER_UNREGISTERED \
	-1 /** User unregistered (not registered but with services for unregistered state) */
...
#define IFC_TERMINATING_UNREGISTERED \
	2 /** Session case terminating to unregistered user*/
...
#define IFC_SESSION_CONTINUED \
	0 /** Session should continue on failure to contact AS */
#define IFC_SESSION_TERMINATED \
	1 /** Session should be terminated on failure to contact AS */
...
#define VALID_CONTACT(c, t)                                                 \
	(((c->expires > t) || (c->expires == 0)) && c->state != CONTACT_DELETED \
			&& c->state != CONTACT_DELETE_PENDING                           \
			&& c->state != CONTACT_EXPIRE_PENDING_NOTIFY                    \
			&& c->state != CONTACT_DELAYED_DELETE                           \
			&& c->state != CONTACT_NOTIFY_READY)

#define VALID_UE_TYPE(c, t) \
	((t == 0) || (t == 1 && c->is_3gpp) || (t == 2 && !c->is_3gpp))
...
{
	ims_public_identity
			*public_identities;			  /**< array of public identities		*/
	...
	ims_cn_service_auth
			*cn_service_auth;		   /**< core net. services auth. 0..1	*/
...
typedef struct ims_subscription_s
{
	...
	int wpsi;	   /** This is not in the standards
	 	 	 	 	 	 	 	 	 	 	 ...
	 	 	 	 	 	 	 	 	 	 	 **/
	ims_service_profile
			*service_profiles;			 /**< array of service profiles		*/
...
typedef struct contact_dialog_data
{
	...
	struct contact_dialog_data
			*prev; /*!< Previous contact in the linked list */
...
{
	gen_lock_t *
			lock; /**< we have to lock the contact as it is shared by many impu structs and has reference counting	*/
	struct contact_hslot
			*slot;	 /*!< Collision slot in the hash table array we belong to */
...
typedef struct impurecord
{
	...
	reg_subscriber *shead,
			*stail; /**< list of subscribers attached			*/
...
typedef int (*insert_impurecord_t)(struct udomain *_d, str *public_identity,
...
typedef int (*get_impurecord_t)(
		struct udomain *_d, str *_aor, struct impurecord **_r);

typedef int (*delete_impurecord_t)(
		struct udomain *_d, str *_aor, struct impurecord *_r);

typedef int (*update_impurecord_t)(struct udomain *_d, str *public_identity,
		impurecord_t *impu_rec, int reg_state, int send_sar_on_delete,
...
typedef int (*update_ucontact_t)(
		struct impurecord *_r, struct ucontact *_c, struct ucontact_info *_ci);

typedef int (*expire_ucontact_t)(struct impurecord *_r, struct ucontact *_c);

typedef int (*unlink_contact_from_impu_t)(struct impurecord *_r,
		struct ucontact *_c, int write_to_db, int is_explicit);

typedef int (*link_contact_to_impu_t)(
		struct impurecord *_r, struct ucontact *_c, int wirte_to_db);

typedef int (*insert_ucontact_t)(struct impurecord *_r, str *_contact,
		struct ucontact_info *_ci, struct ucontact **_c);

typedef int (*delete_ucontact_t)(struct ucontact *_c);

typedef int (*get_ucontact_t)(
		str *_c, str *_callid, str *_path, int _cseq, struct ucontact **_co);

typedef void (*release_ucontact_t)(struct ucontact *_c);

typedef int (*add_dialog_data_to_contact_t)(
		struct ucontact *_c, unsigned int h_entry, unsigned int h_id);

typedef int (*remove_dialog_data_from_contact_t)(
		struct ucontact *_c, unsigned int h_entry, unsigned int h_id);
...
typedef int (*update_subscriber_t)(impurecord_t *urec,
		reg_subscriber **_reg_subscriber, int *expires, int *local_cseq,
		int *version);

typedef void (*external_delete_subscriber_t)(
		reg_subscriber *s, udomain_t *_t, int lock_domain);

typedef int (*get_subscriber_t)(impurecord_t *urec, str *watcher_contact,
		str *presentity_uri, int event, reg_subscriber **reg_subscriber);

typedef int (*add_subscriber_t)(impurecord_t *urec,
		subscriber_data_t *subscriber_data, reg_subscriber **_reg_subscriber,
		int db_load);

typedef int (*get_impus_from_subscription_as_string_t)(udomain_t *_d,
		impurecord_t *impu_rec, int barring, str **impus, int *num_impus,
		int is_shm);

typedef str (*get_presentity_from_subscriber_dialog_t)(
...
typedef struct usrloc_api
{
	...
	get_impus_from_subscription_as_string_t
