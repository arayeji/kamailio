...
int cdp_event_latency =
		1; /*flag: report slow processing of CDP callback events or not - default enabled */
int cdp_event_threshold =
		500; /*time in ms above which we should report slow processing of CDP callback event - default 500ms*/
int cdp_event_latency_loglevel =
		0; /*log-level to use to report slow processing of CDP callback event - default ERROR*/
...
static int mod_init(void)
{
	...
	*callback_singleton = 0;
	...
	LM_DBG("Diameter RX interface successfully bound to TM, Dialog, Usrloc and "
		   "CDP modules\n");
...
void callback_for_cdp_session(int event, void *session)
{
	...
	if(event == AUTH_EV_SESSION_TIMEOUT
		...
		LOG(L_DBG,
				"callback_for_cdp session(): called with event %d and session "
				"id [%.*s]\n",
				event, rx_session_id->len, rx_session_id->s);

		//create new event to process async
		cdp_cb_event_t *new_event =
				new_cdp_cb_event(event, rx_session_id, p_session_data);
		if(!new_event) {
			...
			case IMS_Gq:
				switch(request->commandCode) {
					...
					case IMS_ASR:
						...
						break;
					default:
						LM_ERR("Rx request handler(): - Received unknown "
							   "request for Rx/Gq command %d, flags %#1x "
				...
				break;
			default:
				LM_ERR("Rx request handler(): - Received unknown request for "
					   "app %d command %d\n",
		...
		struct dlg_cell *dlg, int type, struct dlg_cb_params *params)
{
	...
	if(type == DLGCB_TERMINATED || type == DLGCB_DESTROY
			|| type == DLGCB_EXPIRED || type == DLGCB_FAILED) {
		LM_DBG("Dialog has ended or failed - we need to terminate Rx bearer "
			   "session\n");

		LM_DBG("Received notification of termination of dialog with Rx session "
			   "ID: [%.*s]\n",
		...
		if(p_session_data->first_new_flow_description) {
			...
			p_session_data->first_current_flow_description =
					p_session_data->first_new_flow_description;
			//point the new to 0
			LM_DBG("Point the first new fd to 0\n");
			p_session_data->first_new_flow_description = 0;
		} else {
			LM_ERR("There is no new flow description - this shouldn't "
				   "happen\n");
		}
		...
		if(msg->first_line.type == SIP_REPLY) {
			...
			if(msg->cseq && (parsed_cseq = get_cseq(msg))
					&& memcmp(parsed_cseq->method.s, match_cseq_method.s,
							   match_cseq_method.len)
							   == 0) {
				...
				if(msg->first_line.u.reply.statuscode == 200) {
					...
					if(p_session_data->first_new_flow_description) {
						...
						LM_DBG("There is no new flow description - duplicate "
							   "dialog callback - we ignore.\n");
					}
				} else if(msg->first_line.u.reply.statuscode > 299) {
					LM_DBG("Response is more than 299 so this is an error "
						   "code\n");
					...
					if(new_has_video && !current_has_video) {
						LM_DBG("New flow description has video in it, and "
							   "current does not - this means we added video "
...
static int get_identifier(str *src)
{
	...
	if(sep)
		identifier.len = (int)(sep - identifier.s);
		...
		int id_type, int cfg_type)
{
	...
	saved_t_data =
			(saved_transaction_t *)shm_malloc(sizeof(saved_transaction_t));
	if(!saved_t_data) {
		LM_ERR("Unable to allocate memory for transaction data, trying to send "
			   "AAR\n");
		return result;
	}
	...
	if(!auth_session) {
		...
		if(s_id.len > 0 && id_type > -1) {
			...
			LM_DBG("Passed in subscription_id [%.*s] and subscription_id_type "
				   "[%d]\n",
			...
			if(dlg_direction == DLG_MOBILE_ORIGINATING) {
				...
				if(uri.len == 0) {
					LM_ERR("No P-Asserted-Identity hdr found in request. Using "
						   "From hdr in req - we shouldn't have to do this");

					if(!cscf_get_from_uri(orig_sip_request_msg, &uri)) {
						LM_ERR("Error assigning P-Asserted-Identity using From "
							   "hdr in req");
						goto error;
					}
					LM_DBG("going to remove parameters if any from identity: "
						   "[%.*s]\n",
							uri.len, uri.s);
					get_identifier(&uri);
			...
			if(strncasecmp(identifier.s, "tel:", 4) == 0) {
				...
				identifier_type =
						AVP_Subscription_Id_Type_SIP_URI; //default is END_USER_SIP_URI
			}
		}
		//IP
		...
		if(dlg_direction == DLG_MOBILE_ORIGINATING) {
			...
			LM_DBG("IP retrieved from Request SDP to use for framed IP "
				   "address: [%.*s]",
					ip.len, ip.s);

			if(ip.len <= 0) {
				LM_DBG("Request SDP connection IP could not be retrieved, so "
					   "we use SDP stream IP");
			...
			LM_DBG("IP retrieved from Reply SDP to use for framed IP address: "
				   "[%.*s]",
					ip.len, ip.s);

			if(ip.len <= 0) {
				LM_DBG("Reply SDP connection IP could not be retrieved, so we "
					   "use SDP stream IP");
		...
		struct sip_msg *msg, char *route, char *str1, int cfg_type)
{
	...
	char *p;
	int aar_sent = 0;
	saved_transaction_local_t *local_data =
			0; //data to be shared across all async calls
	...
	if(saved_t_data->lock == NULL) {
		LM_ERR("unable to allocate init lock for saved_t_transaction reply "
			   "counter\n");
		return CSCF_RETURN_ERROR;
	}
	...
	for(h = msg->contact; h; h = h->next) {
		if(h->type == HDR_CONTACT_T && h->parsed) {
			for(c = ((contact_body_t *)h->parsed)->contacts; c; c = c->next) {
				...
				if(ul.get_pcontact(domain_t, &contact_info, &pcontact, 0)
					...
					if(pcontact->rx_session_id.len > 0
							...
							&& (auth = cdpb.AAAGetAuthSession(
										pcontact->rx_session_id))) {
						LM_DBG("Rx session already exists for this user\n");
						if(memcmp(pcontact->rx_session_id.s, auth->id.s,
								   auth->id.len)
								!= 0) {
							LM_ERR("Rx session mismatch for rx_session_id "
								   "[%.*s].......Aborting\n",
						...
						LM_DBG("Creating new Rx session for contact <%.*s>\n",
								pcontact->aor.len, pcontact->aor.s);
						int ret = create_new_regsessiondata(domain_t->name,
								&pcontact->aor, &recv_ip, ip_version, recv_port,
								recv_proto, &vb->host, vb->port, vb->proto,
								&rx_regsession_data_p);
						if(!ret) {
							LM_ERR("Unable to create regsession data parcel "
								   "for rx_session_id [%.*s]...Aborting\n",
									pcontact->rx_session_id.len,
									pcontact->rx_session_id.s);
							ul.unlock_udomain(
						...
						auth = cdpb.AAACreateClientAuthSession(1,
								callback_for_cdp_session,
								rx_regsession_data_p); //returns with a lock
						if(!auth) {
							LM_ERR("Rx: unable to create new Rx Reg Session "
								   "for rx_session_id is [%.*s]\n",
									pcontact->rx_session_id.len,
									pcontact->rx_session_id.s);
							if(rx_regsession_data_p) {
								shm_free(rx_regsession_data_p);
					...
					int local_data_len = sizeof(saved_transaction_local_t)
										 + c->uri.len + auth->id.len
										 + vb->host.len + recv_ip.len;
					local_data = shm_malloc(local_data_len);
					if(!local_data) {
						LM_ERR("unable to alloc memory for local data, trying "
							   "to send AAR Register\n");
						lock_release(saved_t_data->lock);
						goto error;
					}
					...
					ret = rx_send_aar_register(msg, auth,
							local_data); //returns a locked rx auth object
					...
					LM_DBG("This contact exists and is not in state REGISTER "
						   "PENDING - we assume re (or de) registration and "
						   "ignore\n");
					ul.unlock_udomain(domain_t, &vb->host, via_port, via_proto);
			...
			if(h->type
					== HDR_CONTACT_T) { //means we couldnt parse the contact - this is an error
